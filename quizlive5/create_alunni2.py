...
def create_files(nomeclasse):
  ...
  <h1>4ae 2021 202 Console Insegnante</h1>
  ...
  <!---------------------- DOVE INSERISCI LA DOMANDA  ------------------>
   ...
   <textarea id="inp1" cols=128 rows=30></textarea><br>
<button onclick="domanda_insegnante4ae.innerHTML=inp1.value">Mostra domanda</button>
<button onclick="domanda_insegnante4ae.innerHTML=''">Nascondi domanda</button>
  <div id="domanda_insegnante4ae">...</div>
  ...
  // var dbdomanda = firebase.database().ref().child("domanda").child("insegnante4ae");
  // dbdomanda.on("value", snap => domanda_insegnante5ce.innerHTML = snap.val());
  // dbdomanda.on("value", snap => inp1.value = snap.val())
  ...
  html1 = html1.replace("4ae", nomeclasse)
