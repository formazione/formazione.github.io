...
def draw_progress_bar(surface, correct_answers, total_questions):
    for i in range(total_questions):
        color = GREEN if i < correct_answers else RED if i < current_question_index else BLUE
...
while running:
    ...
    if current_question_index < TOTAL_QUESTIONS:
        draw_text(f"Domanda {current_question_index + 1}/{TOTAL_QUESTIONS}", font, BLACK, screen, WIDTH // 2, 50)
        draw_text(f"{num1} + {num2} = ?", font, BLACK, screen, WIDTH // 2, 150)
        draw_text(f"Livello {level}", font, BLACK, screen, WIDTH - 100, 50)
        draw_text(f"Tempo rimasto: {int(time_left)}s", font, BLACK, screen, WIDTH // 2, 200)
