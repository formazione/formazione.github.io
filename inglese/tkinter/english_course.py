...
# ===================================================================================
# 1. CENTRALIZED LESSON DATA
# Each lesson now has a 'course' key to define its category.
# ===================================================================================
LESSON_DATA = {
    'present_simple': {
...
# Define which lessons belong to each full course.
# ===================================================================================
COURSES = {
...
# ===================================================================================
# 3. GENERIC PRACTICE MODULE (Unchanged)
# ===================================================================================
class PracticeModule(tk.Toplevel):
    ...
    def __init__(self, parent, lesson_key):
        ...
        self.geometry("700x600"); self.configure(bg='#f4f4f4'); self.resizable(False, False); self.grab_set()
        self.entries = []
        self.create_widgets()
    ...
    def check_answers(self):
        ...
        for i, entry in enumerate(self.entries):
            ...
            if user_answer == correct_answer: entry.config(fg='green', bg='#e6ffe6')
            else: all_correct = False; entry.config(fg='red', bg='#ffe6e6')
        if all_correct: messagebox.showinfo("Great Job!", "🎉 All answers are correct!", parent=self)
        else: messagebox.showwarning("Review Needed", "Some answers are incorrect.", parent=self)

# ===================================================================================
# 4. MAIN APPLICATION
# Re-architected to use the dynamic, listbox-driven UI.
# ===================================================================================
class EnglishLearningApp:
    ...
    def show_welcome_slide(self):
        ...
        tk.Label(welcome_frame, text="Learning Dashboard 🚀", font=('Arial', 22, 'bold'), bg='white', fg='#0056b3').pack(pady=(10, 20))
        ...
        # Right column: Single Practice Topics
    ...
    def update_practice_list(self, event=None):
        ...
        self.practice_list_keys = lesson_keys # Store keys in order
        for key in lesson_keys:
            self.practice_listbox.insert(tk.END, LESSON_DATA[key]['title'])

    def start_selected_course(self):
        selection_indices = self.course_listbox.curselection()
        if not selection_indices:
            messagebox.showwarning("No Selection", "Please select a course from the list to start.")
            return
    ...
    def launch_practice_module(self, event=None):
        ...
        selected_lesson_index = selection_indices[0]
        lesson_key = self.practice_list_keys[selected_lesson_index]
        PracticeModule(self.root, lesson_key)
    ...
    def check_answers(self, lesson_key):
        correct_answers = LESSON_DATA[lesson_key]['answers']; all_correct = True
        for i, entry in enumerate(self.current_entries):
            user_answer = entry.get().strip().lower(); correct_answer = correct_answers[i].lower()
            if user_answer == correct_answer: entry.config(fg='green', bg='#e6ffe6')
            else: all_correct = False; entry.config(fg='red', bg='#ffe6e6')
        if all_correct: messagebox.showinfo("Great Job!", "🎉 All answers are correct!")
        else: messagebox.showwarning("Review Needed", "Some answers are incorrect.")
