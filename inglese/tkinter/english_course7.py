...
# - Audio is now generated into an in-memory BytesIO buffer.
# - A temporary, auto-deleting file is used for playback to ensure compatibility.
# ===================================================================================
...
def play_audio_from_memory(audio_buffer: io.BytesIO):
    """
    Plays audio from an in-memory BytesIO buffer.
    To ensure cross-platform compatibility with the 'playsound' library,
    the buffer's content is written to a named temporary file which is
    automatically deleted after playback.
    """
    try:
        audio_buffer.seek(0)
        # Using a temporary file is the most reliable way to get playsound
        # to work across all platforms (especially Windows) without extra dependencies.
        with tempfile.NamedTemporaryFile(delete=True, suffix=".mp3") as fp:
            fp.write(audio_buffer.read())
            playsound(fp.name)
    except Exception as e:
...
def speak(text_to_speak: str):
    """
    Main function to handle TTS. It runs the async generator in a separate
    thread to avoid freezing the Tkinter GUI.
    """
...
class PracticeModule(tk.Toplevel):
    ...
    def create_widgets(self):
        ...
        for i, q_text in enumerate(self.lesson_data['questions']):
            ...
            # --- Hear Button (Calls the global 'speak' function) ---
