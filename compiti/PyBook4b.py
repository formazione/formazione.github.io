...
class Ebook:
    def __init__(self, root):
        ...
        # Add files (it is called when user press ctrl+l to hide/show left menu)
        ...
        # Select first element of the list in the left menu
        self.lstb.select_set(0)
        # Memorize the name of the active element (0 at start)
        self.filename = self.lstb.get("active")
        # Memorize the index of the current selection
        self.index = self.lstb.curselection() # add for hide *********
        self.start = 1 # This is to avoid deleting the first file text at start
    ...
    def menu(self):
        ...
        self.menubar.add_command(label="+", command = lambda: self.new_window(NewTxt))
    ...
    def new_chapter(self, filename):
    	# Close the window to input name of new txt file
        self.new.destroy()
        # If user inputs wiht no .txt it add to the the name
    ...
    def show_text_in_editor(self):
        """Shows text of selected file in the editor"""
       	if self.start == 1:
            self.save()
        print("Attuale file selezionato", self.filename)
        if self.start == 0:
            # apre il file selezionato e confronta il contenuto con il testo nel widget
            with open(self.filename) as file:
                file = file.read()
                # Se sono uguali, non salva
                testowid = self.text.get("1.0", tk.END)[:-1]
                uguali = file == testowid 
                if uguali:
                    # print("Uguali?", uguali)
                    # print("file ", self.filename, len(file.encode("utf-8")))
                    # print("testo widget ", len(testowid.encode("utf-8")))
                    ...
                    # print("file ", self.filename, len(file.encode("utf-8")))
                    # print("testo widget ", len(testowid.encode("utf-8")))
                    # # Se ci sono novità salva
                    print("Salvato file: ", self.filename)
                    self.save()
        self.index = self.lstb.curselection() # add for hide ******
...
class NewTxt:
	def __init__(self, root):
		self.master = root
		tk.Label(self.master, text="File name:").grid(row=0)
		self.e1 = tk.Entry(self.master)
		self.e1.grid(row=0, column=1)
		self.e1.focus()
		self.master.bind("<Return>", lambda x: app.new_chapter(self.e1.get()))
...
class Help:
    def __init__(self, root):
        ...
        istruzioni = """
Keyboard Shortcuts
ctrl + b .......... publich ebook
ctrl + + - ........ increase / decrease letters
ctrl + Mousewheel .. increase / decrease letters
ctrl + p .......... render active page
ctrl + l .......... show / hide left menu

Markup to use with #html_concert tag in the page:
* H2
^ H3
§ <img src=...
=> red
