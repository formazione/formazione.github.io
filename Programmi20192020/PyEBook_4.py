...
class Ebook:
    ...
    def filelist(self):
        ...
        self.files = glob.glob(f"{folder}\\*.txt")
    ...
    def reload_list_files(self, filename=""):
        ...
        self.files = [f for f in glob.glob(f"{folder}\\*txt")]
        for file in self.files:
            self.lstb.insert(tk.END, file)
        self.lstb.select_set(self.files.index(f"{folder}\\" + filename))
    ...
    def rename(self, filename):
        ...
        if os.path.exists(filehtml):
            os.rename(filehtml , f"{folder}\\" + filename[:-4] + ".html")
        self.lstb.delete("active")
        os.rename(self.filename, f"{folder}\\" + filename)
        self.files = glob.glob(f"{folder}\\*.txt")
        self.reload_list_files(filename)
    ...
    def create_newlinks(self):
        ...
        os.startfile("{}\\{}.html".format(folder, self.current))
    ...
    def show_text_in_editor(self):
        ...
        if self.text.get("1.0", tk.END) != "\n":
            if self.content == self.text.get("1.0", tk.END):
...
if __name__ == "__main__":
    ...
    root = tk.Tk()
    app = Ebook(root)
    app.root.title("PyEBook 4 " + classe)
    root.mainloop()