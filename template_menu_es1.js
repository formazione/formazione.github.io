let prefissopagine = "template_es"; // vai a button(es)
...
ref.on('value', function(snapshot) {
    ...
    snapshot.forEach(function(childSnapshot) {
      let div1 = document.getElementById("al" + count1);
      var childData = childSnapshot.val();
      console.log("Ciao");
      div1.innerText = childData;
      count1++;
    });
...
ref.child(classe[cl_cnt++]).on("value", snap => al15.innerText = snap.val());
  ...
  ref.child(classe[cl_cnt++]).on("value", snap => al22.innerText = snap.val());
    ref.child(classe[cl_cnt++]).on("value", snap => al23.innerText = snap.val());
...
let classe = ['john','jack','peter','jimmy','sammy',
              'carl','enyt','basil','bent','bolt',
              'benny','sammy','royal','tree','aries',
              'sabs','alyn','kurt','klein','russel',
             'keyth','ross',"hjhjhJ","llll"
             ] 
